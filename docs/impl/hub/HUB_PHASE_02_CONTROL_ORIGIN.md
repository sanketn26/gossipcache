# Hub P2 — Control-stream origin (gRPC)
...
- [ ] Serve gRPC `Control.Connect` (bidirectional stream) on the hub listener.
...
### Stream origin (`internal/control` + `internal/l2`, hub side)
...
gRPC `Control.Connect` maps one connection to one authenticated node. Server
sends `InvalidationBatch` / `StreamCheckpoint` / `ReplayUnavailable`; client
sends `Hello` / `Subscribe` / `StreamAcknowledgement` / `ReplayRequest` /
`InvalidateConfirm` (see common P2). Conversion: `internal/control`.
...
type subscriber struct {
    ...
    // stream send is the gRPC server-stream writer for this node
}
```
...
- After commit eligibility, the event is appended to `ring` and fanned out to
  each subscriber's `sendQ`. A full `sendQ` **drops that subscriber** (cancel
  stream + mark for reconnect) — it never blocks the partition commit.
...
- Dropping/disconnecting a subscriber cannot make an event eligible for early
  reclamation.
- Application watermarks release per-connection delivery buffers and provide
  lag metrics; they do not shorten the shared replay window in v1.
- v1 has no subscriber relay role and no separate subscription lease.
...
The `Mutate` RPC registers the waiter under the event's `stream_sequence`
before the event is published (closing the confirm-before-waiter race).
`InvalidateConfirm` messages add `node_id` to `seen`; reaching `need` completes
the RPC. On deadline, the RPC returns `ErrWriteConfirmTimeout` — the commit is
untouched.
...
options is rejected terminally.

## Verification

- [ ] Ordered publish and subscriber-isolation unit tests.
- [ ] Gap/replay, expired replay and silent-idle checkpoint tests over gRPC.
