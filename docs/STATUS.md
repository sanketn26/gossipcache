...
| Shared frame primitives | `internal/frame` | Encoder/decoder, CRC32C header seal/verify, stream read/write helpers used by control and rpc |
| P2 control protocol | `internal/control` | Fail-closed framed codec, all nine bounded message schemas, reconnect/replay/ack/checkpoint contracts, validation, byte-copy rules and golden vectors; no stream origin or consumer yet |
| P3 data RPC contract | `internal/rpc` | Length-prefixed frames with correlation ID + CRC32C; handshake/profile/status, Get, Set/Delete mutation schemas; status classification helpers; dedup fingerprint/key contracts; golden vectors for Get/Set/Delete, tombstone, `NOT_CAUGHT_UP`, W timeout and every terminal mutation status. No RPC server, client transport, Hub store, or WAL yet |
...
| P2 | Hub stream origin and Node consumer: partition streams, interest, replay retention, checkpoint freshness and W aggregation (common framed protocol is implemented) |
| P3 remainder | Memory Hub store + RPC server; Node RPC client; opt-in durability/recovery profile and WAL golden vectors |
